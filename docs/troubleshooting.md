...
`boundary` needs a non-`leaf`, non-`implicit` boundary. `compat` needs a
`version_source`. `behavior` needs declared behavior paths. Either add the
input or gate only facets the component supplies.
